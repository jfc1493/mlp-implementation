...
if __name__ == '__main__':
    ...
    start = time.time()
    nn.fit(mnist.train_data, mnist.train_labels, K=100, epochs=1,
           fast=False)  # 1 epoch et backprop avec boucle (fast=false)
    stop = time.time()
    print("Avec boucle: " + str(stop-start) + "s")

    start = time.time()
    nn.fit(mnist.train_data, mnist.train_labels, K=100, epochs=1,
           fast=True)  # 1 epoch et backprop avec calcul matriciel
    stop = time.time()
    ...
    nn.fit(mnist.train_data, mnist.train_labels, K=128, epochs=25,
           fast=True, lrate=0.005, wd=0.000015, print_stats=True)
