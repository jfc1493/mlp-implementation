...
class NN:
    ...
    def fit(self,x,y, K=1, epochs=25, lrate=0.03, wd=0.0001, fast=False, print_stats=False):    
        ...
        for ep in range(epochs):
            for i in range(len(y)/K):
                ...
                else: grads = self.bprop_fast(x[i*K:i*K+K,:],y[i*K:i*K+K])
        ...
        if(print_stats):    
            plot_stats(train_err,valid_err,test_err,train_cost,valid_cost,test_cost)
    ...
    def compute_loss(self,x,y):
        loss= np.sum(-np.log(self.compute_predictions(x))*onehot(y,self.n_output))
