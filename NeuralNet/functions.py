...
def plot_decision(X, y, nn, res=1):
    ...
    Z = np.argmax(nn.compute_predictions(np.c_[xx1.ravel(), xx2.ravel()]),axis=1)
...
def plot_stats(train_err,valid_err,test_err,train_cost,valid_cost,test_cost):
    ...
    path='/home/jean-francis/Documents/IFT6390/'
    ...
    text=np.transpose((train_err,train_cost,valid_err,valid_cost,test_err,test_cost))
    header="train_err,train_cost,valid_err,valid_cost,test_err,test_cost"
    np.savetxt(path+'Errors and costs', text,fmt='%1.3f',delimiter=",",header=header)
